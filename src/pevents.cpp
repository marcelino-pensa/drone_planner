...
namespace pevents
{
	...
	event_t CreateEvent(bool manualReset, bool initialState)
	{
		event_t event = new neosmart_event_t_;
	...
	int UnlockedWaitForEvent(event_t event, uint64_t milliseconds)
	{
		int result = 0;
		if (!event->State)
		{
	...
	int WaitForEvent(event_t event, uint64_t milliseconds)
	{
		int tempResult;
		if (milliseconds == 0)
		{
	...
	int WaitForMultipleEvents(event_t *events, int count, bool waitAll, uint64_t milliseconds, int &waitIndex)
	{
